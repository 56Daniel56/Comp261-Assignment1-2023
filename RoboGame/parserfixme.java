...
public class Parser {
    ...
    ExprNode parseExpr(Scanner s){
        ...
        if(s.hasNext(NUMPAT)){
            int num = requireInt(NUMPAT,"requires numberpat",s);


        }
        if(s.hasNext(SENSPAT)){
            require(SENSPAT, "need sens pattern", s);
            SensNode sen = parseSens(s);
        }
        if(s.hasNext(OPPAT)){
            
            OperationNode op = parseOp(s);
        }
    ...
    OperationNode parseOp(Scanner s){
        
        if(s.hasNext("add")){
            require("add", "Op pattern required", s);
            return OperationNode.add;
        }

        if(s.hasNext("sub")){
            require("sub", "Op pattern required", s);
            return OperationNode.sub;
        }

        if(s.hasNext("div")){
            require("div", "Op pattern required", s);
            return OperationNode.div;
        }

        if(s.hasNext("mul")){
            require("mul", "Op pattern required", s);
            return OperationNode.mul;
        }
...
interface OperationNode{
    ...
    OperationNode add = new OperationNode() {
        int v1;
        int v2;
        @Override
        public int evaluate(int a, int b) {
    ...
    OperationNode sub = new OperationNode() {
        int v1;
        int v2;
        @Override
        public int evaluate(int a, int b) {
    ...
    OperationNode div = new OperationNode() {
        int v1;
        int v2;
        @Override
        public int evaluate(int a, int b) {
    ...
    OperationNode mul = new OperationNode() {
        int v1;
        int v2;
        @Override
        public int evaluate(int a, int b) {
